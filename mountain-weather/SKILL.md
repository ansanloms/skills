...
- 複数ソース (てんくら・気象庁・Open-Meteo・推定) が食い違うときは楽観側を採らない。悪い方の見立てを主に置き、両方を併記する。Open-Meteo の標高指定気温とてんくらの気圧面気温が食い違う場合も同じ原則に従い、寒い・強風・荒天側を主に置く。
...
```sh
# $SKILL_DIR はこの skill のディレクトリ。例は富士山頂 (3776m)
deno task --cwd "$SKILL_DIR/scripts" fetch-openmeteo --lat=35.3606 --lon=138.7274 --elevation=3776
```
...
- 山頂の緯度・経度・標高は地図・公的資料から確定して渡し、憶測で置かない。レスポンスの `latitude`/`longitude` はモデル格子に丸めた値が返るが、`elevation` は指定値がそのまま echo される (これを検証に使う)。
...
山頂気温は前節 Open-Meteo の標高指定値を主とする。山頂標高そのもので補正した 1 時間ごとの値が取れるため、気圧面からの内挿・外挿や麓からの概算より直接的である。ただし数値予報の生値であり山頂の実測ではない (「限界をユーザに伝える」参照)。てんくらの気圧面気温と食い違えば寒い側を主に置く。
...
- 取得・抽出に失敗した場合 (`area.json` で都道府県が引けない/`fetch-index` が登山指数 0 件で停止/`fetch-openmeteo` が elevation 不一致で停止/forecast の構造が変わる等) は値を捏造しない。処理を止めて報告し、scripts・skill を更新する。
