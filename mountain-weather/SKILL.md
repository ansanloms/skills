...
気象庁アメダスの `50066` (地点名「富士山」、標高 3775m) は山頂に設置されており、気温・湿度・気圧を 30 分ごとに観測している。富士山を扱うときはこれを山頂気温の実測として使える。

```sh
# <hh> は 00/03/06/09/12/15/18/21 のいずれか (3 時間ごとのファイルに 30 分値が入る)
curl -s "https://www.jma.go.jp/bosai/amedas/data/point/50066/<yyyymmdd>_<hh>.json"
```

- 使いどころは 2 つ。過去・現在の実況として山頂気温を答えること、および予報の当たり外れを事後に確認すること。
- MUST: 風は観測していない (`wind` は常に `null`)。富士山でも山頂の風の実測は取れないので、風は他の山と同じくてんくらの気圧面風を主に扱う。この「気温だけ実測が取れる」非対称を、風にも実測があるかのように書かない。
...
- 岩峰・岩塔の山頂は、標高 API の値が公称標高より数十 m 低く出ることがある。標高データは一定間隔の格子の代表値で、鋭い岩峰は平滑化されて削られるため。低く出ること自体は座標の誤りを意味しないので、数十 m 以内の不足で正しい山頂を捨てない。逆に公称標高より 100m 以上高く出る場合は別の山を指している可能性が高い。
...
- 風の取得元の主はてんくらの気圧面風とする (「1.」の風の規則に従い、2 面を山頂標高で内挿して基準値を出す。強い側をそのまま採らない)。Open-Meteo の `wind_speed_10m` は地形加速が載らず過小になりうるため参考値に留め (「限界をユーザに伝える」参照)、`wind_gusts_10m` は強い側の照合に使う。てんくらの基準値と Open-Meteo の値が食い違う場合、体感補正・行動判断には強い側を採る (このソース間の比較では強い側を採ってよい。禁止しているのはてんくらの 2 気圧面の間で強い側を採ることであって、ソース間の比較ではない)。
