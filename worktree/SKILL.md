...
`<base>` が供給されていない場合、この skill は配置先を自前で決めない。`.claude/worktrees/` 等を勝手に補わず、配置先を供給するよう呼び出し側に求める。求める際は「絶対パスで供給すること」と「`git status` を汚さない場所であることの保証は呼び出し側の責務であること」を併せて伝える (供給値の品質不足による再往復を防ぐ)。
