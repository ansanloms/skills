...
- 「今の渋滞」「現在の所要時間」「リアルタイムの混雑」を求められた場合、この手順では取得できない。まず取得できない旨を明示し、断ったうえで、この skill で出せる範囲 (将来の渋滞予測) を代替として提示する。断るだけで終えない。
...
- 一時ファイルは絶対パスで指定し (例 `-o /tmp/highway.json`)、bash の作業ディレクトリに依存させない。後続の `jq` も同じ絶対パスを参照する。使用後に削除する。
