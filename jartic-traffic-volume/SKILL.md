...
- 特定の観測点だけ取りたい場合は BBOX の代わりに `常時観測点コード=01234567` (8 桁 0 埋め) を指定できる。このとき道路種別も不要になる (コードで一意のため)。置換後の完全形は `cql_filter=時間コード=${TC} AND 常時観測点コード=03310890`。ジオコーディングも省略する。
